...
export class CreateInventoryReceiptComponent {
  ...
  originalInventoryReceipt: InventoryReceipt | undefined
  ...
  ngOnInit(): void {
    this._popupCS.selectPayload$.subscribe((payload) => {
      this.originalInventoryReceipt = payload,
  ...
  save() {
    ...
    if (!this.originalInventoryReceipt) {
      ...
      const updateData = {
        ...
        id: this.originalInventoryReceipt.id,
      }
