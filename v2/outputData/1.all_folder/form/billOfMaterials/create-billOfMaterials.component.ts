...
export class CreateBillOfMaterialsComponent {
  ...
  originalBillOfMaterials: BillOfMaterials | undefined
  ...
  ngOnInit(): void {
    this._popupCS.selectPayload$.subscribe((payload) => {
      this.originalBillOfMaterials = payload,
  ...
  save() {
    ...
    if (!this.originalBillOfMaterials) {
      ...
      const updateData = {
        ...billOfMaterialsData,
        id: this.originalBillOfMaterials.id,
      }
